...
socket2.h/recv 関数でテンポラリ用バッファに RTP パケットを受信．  
recv 関数はパケットの到着まで関数を抜けないため，ロックの外側で処理．  
mutex のロックを取得し，解析スレッドと共有するバッファに，受信したパケットデータをコピー．  
解析スレッドに受信を通知 ( std::condition_variant::notify_all() ) して mutex のロックを解除．

#### 解析スレッド

std::condition_variant::wait 関数でパケットの受信を通知されるまで待機．
通知後，未解析のパケットがあれば std::condition_variant::wait 関数を抜ける．
このとき mutex のロックを取得する．  
未解析のパケットを排出して， mutex のロックを解除．  
排出したパケットを解析処理．
